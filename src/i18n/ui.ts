...
export const UI: Record<Locale, UIStrings> = {
  en: {
    ...
    home: {
      h1: 'WebTools',
      lede: 'Image conversion tools that run right in your browser. No uploads, no install, free.',
  ...
  ko: {
    ...
    home: {
      h1: 'WebTools',
      lede: '브라우저에서 바로 쓰는 이미지 변환 도구. 업로드 없음, 설치 없음, 무료.',
      cardTitle: (f) => `${f} 로 변환`,
    },
    tool: {
      heading: (f) => `${f} 변환기`,
  ...
  zh: {
    ...
    home: {
      h1: 'WebTools',
      lede: '在浏览器中即开即用的图片转换工具。无需上传，无需安装，完全免费。',
      cardTitle: (f) => `转换为 ${f}`,
    },
    tool: {
      heading: (f) => `${f} 转换器`,
  ...
  ja: {
    ...
    home: {
      h1: 'WebTools',
      lede: 'ブラウザですぐ使える画像変換ツール。アップロード不要、インストール不要、無料。',
      cardTitle: (f) => `${f} に変換`,
    },
    tool: {
      heading: (f) => `${f} コンバーター`,
